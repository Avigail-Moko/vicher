...
export class ProductStepperComponent {
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data: any,private formBuild: FormBuilder,private newService: NewService,
...
onFileSelected(event: any) {
  ...
  if (files.length > 0) {
    ...
    // Check file size
      if (files[0].size > 3 * 1024 * 1024) {
        ...
        this.sixthFormGroup.get('image')?.setErrors({ 'sizeLimitExceeded': true });
        // Reset file input
  ...
  createProduct(){
    ...
    validationMessages = {
      image: [
        { type: 'sizeLimitExceeded', message: 'הקובץ עולה על 2 MB' }
      ]}
  }
